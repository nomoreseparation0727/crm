#!/usr/bin/env python3
"""Cron entrypoint: scrape every tracked company's website for team/people,
fund list + fact-sheet holdings, and AUM mentions. See sources/website.py
for the caveat that these extractors are heuristic and likely need tuning
against each real DOM after first deploy -- every fetch is archived to
raw_snapshots precisely so that tuning doesn't require re-scraping.
"""
...
def scrape_team(conn, run_id: int, company_id: int, company: CompanyConfig) -> int:
    html = website.fetch_html(company.team_url)
    _save_raw(conn, run_id, company.team_url, "html", html)

    people = website.parse_team_page(html)
    observed_at = datetime.now(timezone.utc)
    log.info("parsed %d person entries from %s team page", len(people), company.slug)
    ...
    with conn.cursor() as cur:
        for p in people:
            cur.execute(
                ...
                VALUES (%s, %s, %s, %s, %s, %s)
                """,
                (company_id, observed_at, p.name, p.title, p.bio_excerpt, company.team_url),
            )
    ...
    for amount, unit, sentence in website.find_aum_mentions(html):
        _store_aum(conn, company_id, company, amount, unit, company.team_url, sentence)

    return len(people)


def _store_aum(conn, company_id: int, company: CompanyConfig, amount: float, unit: str, source: str, raw_text: str) -> None:
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            RETURNING id
            """,
            (company_id, amount * multiplier, company.aum_currency, source, raw_text),
        )
        snapshot_id = cur.fetchone()[0]
    diff_aum(conn, company_id, snapshot_id)


def scrape_funds(conn, run_id: int, company_id: int, company: CompanyConfig) -> int:
    html = website.fetch_html(company.funds_url)
    _save_raw(conn, run_id, company.funds_url, "html", html)

    funds = website.parse_funds_list(html, company.funds_url)
    log.info("found %d candidate fund links for %s", len(funds), company.slug)
    ...
    for f in funds:
        ...
        for amount, unit, sentence in website.find_aum_mentions(fund_html):
            _store_aum(conn, company_id, company, amount, unit, f.url, sentence)
...
def _run_for_company(conn, company: CompanyConfig) -> None:
    ...
    run_id = start_scrape_run(conn, "website", company_id)
    items_seen = 0
    try:
        items_seen += scrape_team(conn, run_id, company_id, company)
        items_seen += scrape_funds(conn, run_id, company_id, company)
        finish_scrape_run(conn, run_id, "success", items_seen=items_seen)
    except Exception as exc:  # noqa: BLE001
        log.exception("website job failed for %s", company.slug)
        conn.rollback()
